...
def remember_want(user_id, want: str):
    state = _ensure_user_state(user_id)
    want = want.strip()
    if not want:
        return
    wants = state.setdefault("wants", [])
...
def get_memory_context(user_id) -> str:
    ...
    if m.get("wants"):
        parts.append("Saved requests and goals:\n" + "\n".join(f"- {w}" for w in m["wants"][-20:]))
    if m.get("emotions"):
...
def auto_extract_memory(user_id, text: str):
    ...
    # Explicit wants and direction: save requests/goals to memory
    want_match = re.search(
        r"\b(?:remember that i want|remember i want|i want you to|i'd like you to|i want you|please remember to|don't forget to|from now on,?\s*)(.+)",
        lower,
    )
    if want_match:
        start, end = want_match.span(1)
        want_text = text[start:end].strip()
        if want_text:
            remember_want(user_id, want_text)
...
def cmd_start(message):
    bot.send_message(
        ...
        "✍️ /want + text → save a request or goal\n"
        "✍️ /selfupdate + text → tell me how to update myself\n"
        "📌 /wants → show saved requests and goals\n"
...
@bot.message_handler(commands=["want", "selfupdate", "save"])
def cmd_want(message):
    parts = message.text.strip().split(maxsplit=1)
    if len(parts) < 2:
        bot.send_message(message.chat.id, "tell me what you want me to remember or do.\nExample: /want remind me to be funny")
        return
    want_text = parts[1].strip()
    remember_want(message.from_user.id, want_text)
    send_reply(message.from_user.id, message.chat.id, f"got it, i'm saving that as a request: \"{want_text}\"")

@bot.message_handler(commands=["wants"])
def cmd_wants(message):
    u = uid(message.from_user.id)
    wants = _memory.get(u, {}).get("wants", [])
    if not wants:
        bot.send_message(message.chat.id, "i don't have any saved requests yet — tell me with /want")
        return
    bot.send_message(message.chat.id, "📌 saved requests and goals:\n" + "\n".join(f"- {w}" for w in wants[-20:]))
